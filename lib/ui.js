...
logs.on('scroll', function() {
	...
	showDisplay(value, 'Logs');
	screen.render();
});

details.on('scroll', function() {
	var value = details.value;
	showDisplay(value, 'Header-details');
});
...
function showDisplay(content, caller) {
	display.setContent('{center}{bold}FULL-DISPLAY: ' + caller + '{/bold}{/center}\n\n' + content);
}

var addLog = function(log, headers) {
	log = '#' + padLeft(id, 4, 0) + ' : ' + padRight(headers.Host, 30, ' ') + ' ' + log;
	id++;
